...
public final class FoldParserManager implements SyntaxConstants {
    ...
    private FoldParserManager() {
        foldParserMap = new HashMap<String, FoldParser>();
        ServiceLoader<TokenMakerRegistration> s = ServiceLoader.load(TokenMakerRegistration.class);
        for (TokenMakerRegistration l : s) {
            FoldParser fp = l.getFoldParser();
            if (fp != null) {
                foldParserMap.put(l.getLanguage(), fp);
            }
        }
    }
     ...
     * will be shared among all RSTA instances editing that language, so it
     * should be stateless (which should not be difficult for a fold parser).
     * You can also override the fold parser for built-in languages, such as
...
//        map.put(SYNTAX_STYLE_BSH, new CurlyFoldParser(true, true));

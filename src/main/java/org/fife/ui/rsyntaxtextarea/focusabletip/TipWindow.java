...
class TipWindow extends JWindow implements ActionListener {
	...
	void fixSize() {
		...
		try {
			...
			final int MAX_WINDOW_W = (ft.getMaxSize() != null ? (int) ft.getMaxSize().width  : 600);
			final int MAX_WINDOW_H = (ft.getMaxSize() != null ? (int) ft.getMaxSize().height  : 400);
			...
			if (r.y+r.height>d.height) {
				...
				if(ft.getMaxSize() != null) d.height = Math.min(d.height, MAX_WINDOW_H);
				textArea.setPreferredSize(d);
			}
